...
$('.subtitle').addClass('hide-subtitle')
...
setTimeout(() => {
	...
	$('.subtitle').addClass(hideClass).removeClass('hide-subtitle')
...
function scorePoint() {
	...
	if (path[0] === $('.score-points > span')[1]) {
		$(path[2]).html(path[2].innerText.replace('uno', 'dos'))
	}
...
function endGame() {
	...
	if (parseInt($('.score-games > span')[1].innerText) > parseInt(path[0].innerText)) {
		$(path[1]).html(path[1].innerText.replace('uno', 'dos'))
	}
